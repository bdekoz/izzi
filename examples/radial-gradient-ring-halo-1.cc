...
// express two int radius as a string percentage
const string
compute_offset_percentage(const size_type numerator,
			  const size_type denominator)
{
  const double ratio = static_cast<double>(numerator)/static_cast<double>(denominator);
  const size_type perc(round(ratio * 100));
  return to_string(perc) + "%";
}
...
test_radial_gradient(std::string ofile)
{
  ...
  const string outerstr = compute_offset_percentage(radius, oring);
  radial_gradient rgrad3o;
  rgrad3o.start_element(rgrad3o_name);
  rgrad3o.stop(outerstr, color::crimson);
  ...
  const string innerstr = compute_offset_percentage(iring, radius);
  radial_gradient rgrad3i;
  rgrad3i.start_element(rgrad3i_name);
  rgrad3i.stop(innerstr, color::white);
