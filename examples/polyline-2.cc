...
void
make_marker_circle(const std::string id, const svg::area<> ma, const svg::point_2t mcp)
{
  svg::marker_element mkr;
  mkr.start_element(id, ma, mcp);

  svg::circle_element c = make_circle(cp, svg::k::b_style, 4);
  mkr.add_raw(c.str());
  mkr.finish_element();
  obj.add_element(mkr);
}
...
test_polyline_1()
{
  ...
  polyline_element pl = make_polyline(points, styl1, "2", "c4");
  obj.add_element(pl);
}
